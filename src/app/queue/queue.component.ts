...
export class QueueComponent {
  ...
  ngAfterViewInit() {
    ...
    const playing = this.trackList.nativeElement.children[this.queueService.currentQueueIndex]
    ...
    this.queueService.currentTrack$.subscribe({
      next: (track) => {
        const curr = this.trackList.nativeElement.children[this.queueService.currentQueueIndex]
        if(curr) {
  ...
  drop(event: CdkDragDrop<any, any>) {
    console.log(this.queueService.currentQueueIndex, event.currentIndex, event.previousIndex)
    ...
    else if(event.currentIndex >= this.queueService.currentQueueIndex && this.queueService.currentQueueIndex > 0 && this.queueService.currentQueueIndex > event.previousIndex) {
      this.queueService.currentQueueIndex--
    }
    else if(this.queueService.currentQueueIndex <= event.previousIndex && event.currentIndex <= this.queueService.currentQueueIndex && this.queueService.currentQueueIndex < this.queueService.queue.length - 1) {
      this.queueService.currentQueueIndex++
    }
